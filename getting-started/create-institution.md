# Create an Institution in UniFlow

Each university must create an institution in UniFlow. Each institution represents a separate, secure environment with its own users, students, and data.

---

## Before you begin

Make sure you have:
- A UniFlow System Administrator account.  
- The university’s basic information (name, academic calendar, primary contact).  

---

## Procedure

1. Log in to UniFlow.
2. From the main navigation, select **Administration** → **Institutions**.
3. Click **Create Institution**.
   
Result: The **Create Institution** form opens.

4. In the **Create Institution** form, fill in the following fields:

| Field | Description |
|------|-------------|
| Institution name | The official name of the university. |
| Short code | A unique identifier (for example, `UNIV01`). |
| Country | Used for reporting and compliance. |
| Time zone | Determines academic and billing schedules. |
| Primary contact email | Receives system notifications. |

5. Configure the academic calendar.

Select:
- Start and end dates for the academic year.  
- Term structure (Semester, Trimester, or Quarter). 
6. Click **Create Institution**.

Result: UniFlow creates a new tenant and displays a confirmation message.

---

## What to do next

After creating an institution, you can:

- **Set up Users and Roles**  
- **Configure Academic Terms**  
- **Import Student Records**

---
